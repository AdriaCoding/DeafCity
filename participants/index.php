...
<head>
    ...
    <meta name="page-url" content="<?= htmlspecialchars(preview_page_url_meta('participants'), ENT_QUOTES, 'UTF-8') ?>">
...
<div class="preview-participants-page">
    ...
    <div class="participants-grid">
        <?php foreach ($participantNames as $name): ?>
            ...
            $safeHref = preview_participant_home_url($name);
            if ($preview_lang !== 'en') {
                $safeHref .= (strpos($safeHref, '?') !== false ? '&' : '?') . 'lang=' . rawurlencode($preview_lang);
            }
            ?>
            <a
