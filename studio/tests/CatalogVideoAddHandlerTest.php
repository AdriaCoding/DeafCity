...
class CatalogVideoAddHandlerTest extends TestCase
{
    ...
    public function test_uses_form_tags_instead_of_vimeo_when_provided(): void
    {
        $vimeo = $this->createMock(VimeoClient::class);
        ...
        $this->assertTrue($result['ok']);
        $this->assertSame(['custom', 'tags'], $result['video']['tags']);

        $catalog = json_decode(file_get_contents($this->catalogFile), true);
        $this->assertSame(['custom', 'tags'], $catalog['videos'][0]['tags']);
    }
