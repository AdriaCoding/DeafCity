...
Add a top-only height crop at the desktop `≥651px` breakpoint: scale/position the video so it also overflows vertically, with the vertical anchor biased so any cropping only ever removes from the **top** edge, never the bottom. Implement the amount as a single global, easily-tunable CSS custom property (e.g. `--vpc-top-crop`), not a per-video or per-participant setting. Mobile keeps the width crop but must not receive this vertical crop.
...
- [x] A single global CSS custom property controls the top-crop amount at the `≥651px` breakpoint, independent of the existing mobile width-crop aspect-ratio change
...
Current state: mechanism is implemented for desktop and visually checked against
Mahieddine at desktop width. The final `--vpc-top-crop` value remains pending
