<?php
// Run: php8.4 tests/participants_page_test.php
...
pp_assert_contains('/participants', $html, 'navbar includes Participants route');
...
pp_assert_contains('/about', $html, 'navbar includes About route');
...
preg_match_all('~\\?participant=~', $html, $cardMatches);
...
pp_assert_contains('/participants', $homeHtml2, 'Participants nav button on home page');
...
// catalog (index.php's and participants/index.php's own $vpc===null
...
pp_assert_contains(
    ...
    $playerJsSrc,
    'reset dismisses empty-queue overlay before loading a Video'
);
pp_assert_contains(
    '!vimeoPlayer.isReal()',
    $playerJsSrc,
    'play/pause bootstraps lazy Vimeo player when iframe has no embed yet'
);
