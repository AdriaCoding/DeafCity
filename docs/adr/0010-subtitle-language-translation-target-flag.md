...
The selective flag was removed. Every configured Subtitle language is now a translation destination (excluding the Master and languages that already have captions). The `translation_target` field, Llengues orals checkbox, and related Studio APIs no longer exist. Legacy keys in config JSON are ignored on read.
