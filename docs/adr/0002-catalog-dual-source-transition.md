...
The legacy homepage has been retired. `playlists.json` and `videos.json` no longer exist anywhere in the repo or on the server; `data/catalog.json` is the sole Video metadata source for both Studio and the Website. Filtered views (by Sign language, edition, typology, etc.) are computed at runtime from the Catalog — see `js/vimeo_playlist_logic.js` and `lib/catalog_projection.php` — exactly as this ADR anticipated for the post-transition state. There is no remaining dual-source condition to guard, and no code path should treat Playlists as a separately maintained file.
