...
Canonical documentation for the map at `/preview/about/`. **Read this before
...
| File | Role |
|------|------|
| `data/languages.json` | GeoJSON consumed by Leaflet (`preview/js/sign_language_map.js`) |
| `data/languages.meta.json` | Provenance, counts, fallbacks, overrides (not loaded by the map) |
| `preview/scripts/regenerate_languages_json.py` | **Only** supported way to rebuild the dataset |
| `preview/scripts/glottolog-source.json` | Expected Glottolog version, DOI, CLDF download URL |
| `preview/tests/languages_json_validate_test.py` | Run after every regeneration |
| `preview/about/map.php` | Map markup + Glottolog attribution |
| `preview/js/sign_language_map.js` | Legend branches, default visibility |
...
Check `preview/scripts/glottolog-source.json` for the pinned version. When Glottolog
releases a new stable version, update that file, download the new CLDF zip, regenerate,
and update the attribution string in `preview/about/map.php` if the version changed.
...
To change legend labels or default visibility, edit `preview/js/sign_language_map.js`
...
4. Optionally add an assertion in `preview/tests/languages_json_validate_test.py`
...
python3 preview/tests/languages_json_validate_test.py
php8.4 preview/tests/about_page_test.php   # optional smoke test
```

Validate existing output without regenerating:

```bash
python3 preview/scripts/regenerate_languages_json.py --validate-only
```
...
- URL: `https://deaf.city/preview/about/`
- Attribution footer in `preview/about/map.php` (Glottolog version + DEAF.city note)
