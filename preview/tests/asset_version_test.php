<?php
// Run: php8.4 preview/tests/asset_version_test.php
//
...
av_assert(strpos($url, '/preview/js/vimeo_playlist_logic.js?v=') === 0, 'asset URL has the expected path + query shape');
...
foreach (array('home' => $homeHtml, 'about' => $aboutHtml, 'participants' => $participantsHtml) as $pageName => $html) {
    if (preg_match('~/preview/(?:js|css|components)/[a-zA-Z0-9_.\-]+\.(?:js|css)\?v=(?!\d)~', $html)) {
